  ...
  return (
    <div className="fixed inset-0 z-20 flex justify-center bg-black/60 backdrop-blur-sm p-10">
      <div className="w-full max-w-xl bg-surface h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        >
          {fields.map((field) => {
            const isFullWidth = field.gridSpan === 'full' || field.type === 'textarea' || field.type === 'image';
              ...
              >
                ...
                {field.type === 'select' && (
                  <div className="relative">
                    {IconComponent && <IconComponent className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary" size={16} />}
                    <select
                      name={field.name}
                      value={formData[field.name] ?? ''}
                      onChange={handleInputChange}
        ...
        <div className="p-6 border-t border-border bg-surface flex gap-3">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : submitButtonText}
          </button>
        </div>
