...
export const createUserSubscription = catchAsync(async (req: Request, res: Response) => {
  const {
    user_id,
    plan_id,
    amount_override,
  ...
  if (amount_override !== undefined && (typeof amount_override !== "number" || amount_override < 0)) {
    return res.status(400).json({ success: false, message: "amount_override must be a non-negative number." });
  }

  if (duration_override !== undefined && (!Number.isInteger(duration_override) || duration_override <= 0)) {
    return res.status(400).json({ success: false, message: "duration_override must be a positive integer (days)." });
  }

  if (method && !["cash", "gcash", "bank_transfer"].includes(method.toLowerCase())) {
    return res.status(400).json({ success: false, message: "Invalid payment method structure." });
  }
...
export const cancelUserSubscription = catchAsync(async (req: Request, res: Response) => {
...
export const resetUserSubscription = catchAsync(async (req: Request, res: Response) => {
  const targetUserId = Number(req.body.user_id);
...
export const getUserSubscription = catchAsync(async (req: Request, res: Response) => {
  ...
  if (isNaN(userId) || !Number.isInteger(userId) || userId <= 0) {
    return res.status(400).json({ success: false, message: "Invalid user id query structure." });
  }
